//
//  FetchingFavoriteWeathers.swift
//  Meteo
//
...
protocol FetchingFavoriteWeatherDelegate: class {
    func getError(_ error: String, _ isError: Bool)
    func getWeathers(_ weathers: [MainWeather])
}


class FetchingFavoriteWeathers {
    
    var delegate: FetchingFavoriteWeatherDelegate?
    var arrayWeather: [MainWeather] = []
    
    
    init(result: Results<RealmWeatherManager>) {
        retriveMultipleLocation(for: result)
    }
    
    private func retriveMultipleLocation(for locations: Results<RealmWeatherManager>) {
        ...
        for weather in locations {
            ...
            AF.request(url).responseJSON { (response) in
                ...
                case .failure(let error):
                    ...
                    self.delegate?.getError(error.localizedDescription, _: true)
                }
            }
            
            dispatch.notify(queue: .main) {
                self.delegate?.getWeathers(self.arrayWeather)
            }
        }
    }
